...
// Below is the struct we're using to create a request to scytale
type ScytaleRequest struct {
	Payload     []byte
	ContentType string
	TargetURL   string
...
type RequestHandler interface {
	HandleRequest(workerID int, inRequest ScytaleRequest)
}

type ScytaleHandler struct {
	logging.Logger
}

func (ch *ScytaleHandler) HandleRequest(workerID int, inRequest ScytaleRequest) {
...
type ScytaleHealth struct {
	health.Monitor
}

func (ch *ScytaleHealth) IncrementBucket(inSize int) {
	if inSize < 101 {

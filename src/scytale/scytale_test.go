...
func (m *mockHandler) HandleRequest(workerID int, inRequest ScytaleRequest) {
...
func TestScytaleHealth(t *testing.T) {
	assert := assert.New(t)
	...
	t.Run("TestIncrementBucket", func(t *testing.T) {
		for _, data := range testData {
			...
			scytaleHealth := &ScytaleHealth{fakeMonitor}

			scytaleHealth.IncrementBucket(data.inSize)
...
func TestServeHandler(t *testing.T) {
	...
	fakeHandler.On("HandleRequest", mock.AnythingOfType("int"), mock.AnythingOfType("ScytaleRequest")).Return().Once()
	...
	requestSuccessful := func(func(workerID int)) error {
		fakeHandler.HandleRequest(0, ScytaleRequest{})
		return nil
	}

	serverWrapper := &ServerHandler{
		Logger:         logger,
		scytaleHandler: fakeHandler,
		scytaleHealth:  fakeHealth,
